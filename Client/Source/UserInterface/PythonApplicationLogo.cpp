...
int CPythonApplication::OnLogoOpen (char* szName)
{
	...
	if (FAILED (CoCreateInstance (CLSID_SampleGrabber, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (VOID**)&m_pFilterSG)))
	{
		return 0;
	}
...
int CPythonApplication::OnLogoUpdate()
{
	...
	if (m_pGraphBuilder == NULL || m_pFilterSG == NULL || m_pSampleGrabber == NULL || m_pMediaCtrl == NULL || m_pMediaEvent == NULL || m_pVideoWnd == NULL ||
