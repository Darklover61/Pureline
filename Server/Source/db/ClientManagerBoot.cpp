...
bool CClientManager::InitializeMobTable()
{
	...
	while (data.Next())
	{
		...
		sys_log (1, "MOB #%-5d %-24s %-24s level: %-3u rank: %u empire: %d", mob_table->dwVnum, mob_table->szName, mob_table->szLocaleName, mob_table->bLevel, mob_table->bRank,
				 mob_table->bEmpire);
		++mob_table;
	...
	if (isTestFile)
	{
		...
		while (test_data.Next())	//테스트 데이터 각각을 훑어나가며,새로운 것을 추가한다.
		{
			...
			sys_log (0, "MOB #%-5d %-24s %-24s level: %-3u rank: %u empire: %d", mob_table->dwVnum, mob_table->szName, mob_table->szLocaleName, mob_table->bLevel, mob_table->bRank,
					 mob_table->bEmpire);
			++mob_table;
